...
export default function Tasks() {
  ...
  const pending = (data || []).filter((t: any) => t.status === 'PENDING_APPROVAL');
  const others = (data || []).filter((t: any) => t.status !== 'PENDING_APPROVAL');
            ...
            {pending.map((task: any) => (
              <div key={task.id} className="bg-white rounded-lg p-3 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-slate-400">{new Date(task.appointment?.scheduledDate).toLocaleDateString()} — {task.appointment?.type}</p>
                </div>
          ...
          <tbody>
            {others.map((task: any) => (
              <tr key={task.id} className="border-b hover:bg-slate-50">
                ...
                <td className="px-4 py-3"><span className={`text-xs px-2 py-1 rounded-full font-medium ${STATUS_COLORS[task.status] || ''}`}>{task.status.replace('_',' ')}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
