  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        ...
        {unread > 0 && (
          <button onClick={() => markAll.mutate()} className="text-xs text-blue-600 hover:underline border border-blue-200 px-3 py-1 rounded-lg hover:bg-blue-50">
      ...
      {isLoading ? (
        ...
        <div key={n.id}
          className={"bg-white rounded-xl shadow-sm p-4 flex items-start gap-3 cursor-pointer hover:bg-slate-50 " + (!n.isRead ? "border-s-4 border-blue-500" : "")}
          onClick={() => !n.isRead && markOne.mutate(n.id)}>
          <div className={"w-9 h-9 rounded-full flex items-center justify-center text-lg flex-shrink-0 " + (n.isRead ? "bg-slate-100" : "bg-blue-100")}>🔔</div>
