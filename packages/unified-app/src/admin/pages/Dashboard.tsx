  ...
  return (
    ...
    >
      <div className="space-y-3">
        <Field label={t("common.date")} htmlFor="admin-edit-date">
          <Input id="admin-edit-date" type="date" lang="en-GB" dir="ltr" value={form.date} onChange={e => set("date", e.target.value)} />
        </Field>
        <Field label={t("appointments.type")} htmlFor="admin-edit-type">
          <Select id="admin-edit-type" value={form.type} onChange={e => set("type", e.target.value)}>
            <option value="MAINTENANCE">{t("appointments.maintenance")}</option>
            <option value="INSTALLATION">{t("appointments.installation")}</option>
          </Select>
        </Field>
        <Field label={t("common.status")} htmlFor="admin-edit-status">
          <Select id="admin-edit-status" value={form.status} onChange={e => set("status", e.target.value)}>
        ...
        <Field label={t("common.notes")} htmlFor="admin-edit-notes">
          <Textarea id="admin-edit-notes" rows={3} value={form.notes} onChange={e => set("notes", e.target.value)} />
        </Field>
      </div>
    </Modal>
  );
  ...
  return (
    ...
    >
      <div className="space-y-3">
        ...
        <Field label={t("common.date")} htmlFor="admin-quick-date">
          <Input id="admin-quick-date" type="date" lang="en-GB" dir="ltr" value={date} onChange={e => setDate(e.target.value)} />
        </Field>
        <Field label={t("appointments.type")} htmlFor="admin-quick-type">
          <Select id="admin-quick-type" value={type} onChange={e => setType(e.target.value)}>
            <option value="MAINTENANCE">{t("appointments.maintenance")}</option>
            <option value="INSTALLATION">{t("appointments.installation")}</option>
          </Select>
        </Field>
                  ...
                  {items.map((a: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TD><Badge tone={taskTones[a.workStatus] ?? "neutral"} dot>{a.workStatus || a.status}</Badge></TD>
                        <TD>
...
export default function Dashboard() {
  ...
  // Admin reads the whole operation at once, so all eight counters stay on one
    ...
    <div className="space-y-5">
      ...
      <section className="bg-surface border border-line rounded-md">
        <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-line">
          <h3 className="text-sm font-semibold text-fg">{t("dashboard.recentActivity")}</h3>
          {(activity || []).length > 0 && (
            <Button size="sm" variant="secondary" className="text-danger-fg" onClick={() => setConfirmClearAll(true)}>
        ...
        {(activity || []).length === 0 ? (
          ...
          <ul className="divide-y divide-line-subtle">
            {(activity || []).map((a: any) => (
              <li key={a.customerId} className="group flex items-center justify-between gap-3 px-4 py-2.5">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <Button
                    size="sm" variant="ghost" iconOnly
                    onClick={() => deleteActivity.mutate(a.customerId)}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => clearAllActivity.mutate()}
        title={t("dashboard.clearAllConfirm")}
        message={t("dashboard.clearAllDesc")}
        confirmLabel={t("dashboard.clearAll")}
        cancelLabel={t("common.cancel")}
        destructive
        loading={clearAllActivity.isPending}
      />
    </div>
  );
}
