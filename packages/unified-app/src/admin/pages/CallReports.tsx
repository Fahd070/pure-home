        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 flex gap-2 justify-end">
              ...
              <button type="submit"
                disabled={createMutation.isPending || (!unregisteredMode && !form.customerId) || (unregisteredMode && !form.unregisteredName.trim())}
                ...
                {reports.map((r: any) => (
                  <tr key={r.id} className={`border-b transition-colors ${selected.has(r.id) ? "bg-blue-50" : "hover:bg-slate-50"}`}>
                    ...
                    <td className="px-4 py-3 font-medium">
                      ...
                      {!r.customerId && r.unregisteredName && <span className="ms-1 text-xs text-amber-600 bg-amber-50 px-1 rounded">{isAr ? "غير مسجل" : "Unregistered"}</span>}
                    </td>
