...
export default function TaskDetail() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["task", id], queryFn: () => api.get(`/tasks`).then(r => r.data.data.find((t: any) => t.id === id)) });

  const start = useMutation({ mutationFn: () => api.patch(`/tasks/${id}/start`), onSuccess: () => { qc.invalidateQueries({ queryKey: ["tasks"] }); qc.invalidateQueries({ queryKey: ["task", id] }); toast.success(t("common.success")); } });
  const complete = useMutation({ mutationFn: () => api.patch(`/tasks/${id}/complete`, { notes: completeNotes }), onSuccess: () => { toast.success(t("common.success")); navigate("/technician/queue"); } });
  const postpone = useMutation({ mutationFn: () => api.patch(`/tasks/${id}/postpone`, { reason: postponeReason, newDate: postponeDate || undefined }), onSuccess: () => { toast.success(t("common.success")); navigate("/technician/queue"); } });
    ...
    <div className="max-w-lg mx-auto space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
        <div className="flex justify-between items-start">
          ...
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${task.status === "IN_PROGRESS" ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700"}`}>{task.status.replace("_"," ")}</span>
        </div>
        {addr && (
      ...
      {showComplete && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-80 shadow-xl">
            <h3 className="font-semibold mb-3">{t("tasks.confirmComplete")}</h3>
            <textarea value={completeNotes} onChange={e => setCompleteNotes(e.target.value)} placeholder={t("common.notes")} rows={3}
              className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 mb-4" />
            <div className="flex gap-2">
              <button onClick={() => complete.mutate()} disabled={complete.isPending} className="flex-1 bg-green-600 text-white py-2 rounded-lg text-sm hover:bg-green-700 disabled:opacity-50">{complete.isPending ? t("common.loading") : t("common.save")}</button>
              <button onClick={() => setShowComplete(false)} className="flex-1 border py-2 rounded-lg text-sm hover:bg-slate-50">{t("common.cancel")}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPostpone && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-80 shadow-xl">
            ...
            <div className="flex gap-2">
              <button onClick={() => postponeReason.trim() && postpone.mutate()} disabled={!postponeReason.trim() || postpone.isPending} className="flex-1 bg-yellow-500 text-white py-2 rounded-lg text-sm hover:bg-yellow-600 disabled:opacity-50">{postpone.isPending ? t("common.loading") : t("common.save")}</button>
