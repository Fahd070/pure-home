...
export default function Notifications() {
  ...
  // it). Also invalidates the sidebar's own unread-count query (notif-unread-tech)
  // so the red badge clears immediately, not just on its next 30s poll.
  const markAll = useMutation({
    mutationFn: () => api.patch("/notifications/read-all"),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["notifications-tech"] });
      qc.invalidateQueries({ queryKey: ["notif-unread-tech"] });
    },
  });
