  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await api.post("/customers", {
        name, phone, maintenanceCycle, maintenanceFrequency: Number(maintenanceFrequency),
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-6 space-y-4">
        ...
        <div>
          ...
          <textarea value={form.notes} onChange={e => set("notes", e.target.value)} rows={6}
            ...
            className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 resize-y min-h-[120px]" />
        </div>
