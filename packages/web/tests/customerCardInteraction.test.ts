...
describe("customer card interaction", () => {
  it("makes both customer lists open their department detail route while action buttons stop propagation", () => {
    ...
    // The row is still one clickable unit; it now also carries the v4 overdue
    // emphasis, so the props span several lines and are asserted individually.
    expect(admin).toMatch(/emphasis=\{maintenanceRowClass\(c\.maintenancePriority\)\}/);
    expect(admin).toMatch(/onClick=\{\(\) => navigate\(`\/admin\/customers\/\$\{c\.id\}`\)\}/);
    expect(scheduling).toMatch(/emphasis=\{maintenanceRowClass\(c\.maintenancePriority\)\}/);
    expect(scheduling).toMatch(/onClick=\{\(\) => navigate\(`\/scheduling\/customers\/\$\{c\.id\}`\)\}/);
