...
describe('CallReportForm: pre-fill and stale-context safety', () => {
  it('pre-fills the customer context and locks out unregistered mode when a presetCustomer is given', async () => {
    ...
    // callDate/employeeName inputs still exist; no search input or "unregistered customer" toggle when the customer is already known.
    expect(el.querySelector('input[type="datetime-local"]')).not.toBeNull();
  ...
  it('submits with the correct customerId for the customer currently loaded (not a stale one)', async () => {
    ...
    const dateInput = el.querySelector('input[type="datetime-local"]') as HTMLInputElement;
    const nativeSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value')!.set!;
    act(() => { nativeSetter.call(dateInput, '2026-08-09T10:00'); dateInput.dispatchEvent(new Event('input', { bubbles: true })); });
