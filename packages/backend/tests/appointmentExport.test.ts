...
// Modification #5: Scheduling/Maintenance export -> Admin approval -> Technician workflow.
describe('Appointment export-to-technician approval workflow', () => {
  ...
  // 1. Export success + resulting field values
  it('Scheduling can export an appointment; sets visibleToTechnician=false, adminApproved=false', async () => {
  ...
  // 2. Appointment stays visible to Scheduling while pending
  it('exported/pending appointment remains visible to Scheduling', async () => {
    const appt = await createSchedulingAppointment();
    await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/export-to-technicians`).set('Authorization', `Bearer ${schedToken}`).send({});
    const res = await request(ts.baseUrl).get(`/api/appointments/${appt.id}`).set('Authorization', `Bearer ${schedToken}`);
    expect(res.status).toBe(200);
  });

  // 3. Technician cannot retrieve pending appointment (single GET)
  it('Technician cannot GET a pending-export appointment directly', async () => {
    const appt = await createSchedulingAppointment();
    await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/export-to-technicians`).set('Authorization', `Bearer ${schedToken}`).send({});
  ...
  // 3b. Technician cannot retrieve pending appointment (list)
  it('Technician list does not include a pending-export appointment', async () => {
    const appt = await createSchedulingAppointment();
    await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/export-to-technicians`).set('Authorization', `Bearer ${schedToken}`).send({});
  ...
  it('Admin can approve a pending export; sets visibleToTechnician=true, adminApproved=true', async () => {
    const appt = await createSchedulingAppointment();
    await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/export-to-technicians`).set('Authorization', `Bearer ${schedToken}`).send({});
  ...
  // 7. Ordinary/never-exported appointments remain technician-visible by default (regression)
  it('an ordinary, never-exported appointment stays technician-visible (default true)', async () => {
    const appt = await createSchedulingAppointment();
    const res = await request(ts.baseUrl).get(`/api/appointments/${appt.id}`).set('Authorization', `Bearer ${techToken}`);
    expect(res.status).toBe(200);
    expect(res.body.data.visibleToTechnician).toBe(true);
  ...
  // 8. Duplicate export cannot corrupt state
  it('a second export attempt on an already-pending appointment is rejected (409 ALREADY_PENDING)', async () => {
    const appt = await createSchedulingAppointment();
    await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/export-to-technicians`).set('Authorization', `Bearer ${schedToken}`).send({});
  ...
  it('an export attempt on an already-approved/exported appointment is rejected (409 ALREADY_APPROVED)', async () => {
    const appt = await createSchedulingAppointment();
    await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/export-to-technicians`).set('Authorization', `Bearer ${schedToken}`).send({});
    await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/approve-export`).set('Authorization', `Bearer ${adminToken}`).send({});
    const res = await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/export-to-technicians`).set('Authorization', `Bearer ${schedToken}`).send({});
  ...
  it('approving an appointment that was never exported is rejected (409 NOT_PENDING)', async () => {
    const appt = await createSchedulingAppointment();
    const res = await request(ts.baseUrl).patch(`/api/appointments/${appt.id}/approve-export`).set('Authorization', `Bearer ${adminToken}`).send({});
    expect(res.status).toBe(409);
    expect(res.body.error).toBe('NOT_PENDING');
