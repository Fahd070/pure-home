...
describe('Appointment core flow', () => {
  ...
  it('technician1 completes the appointment', async () => {
    const res = await request(ts.baseUrl)
      ...
      .set('Authorization', `Bearer ${tech1Token}`)
      .send({ serviceDetails: 'Serviced 3 units', completionAmount: 250, completionPaymentMethod: 'CASH', actualCompletionDate: new Date().toISOString().slice(0, 10), technicianName: 'Ahmed' });
    expect(res.status).toBe(200);
